...
class StateEncoder(nn.Module):
    def __init__(self, hidden_dim: int,
        ...
        self.fusion_pwConv = nn.Conv2d(in_channels=2, out_channels=hidden_dim, kernel_size=1, bias=False)
        self.fusion_blocks = nn.Sequential(*[ConvNextBlock(hidden_dim) for _ in range(block_layers // 2)])
        ...
        self.fusion_fc = nn.Linear(in_features=hidden_dim * 6 * 6, out_features=embedding_dim)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        fusion_rep = self.fusion_pwConv(x)
        fusion_rep = self.fusion_blocks(fusion_rep)
        fusion_rep = self.gap(fusion_rep)
        fusion_rep = fusion_rep.view(fusion_rep.size(0), -1)
        fusion_rep = self.dropout(fusion_rep)
        fusion_rep = self.fusion_fc(fusion_rep)
