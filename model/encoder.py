...
class StateEncoder(nn.Module):
    def __init__(self, input_size: int,
        ...
        super().__init__()
        self.agent_pwConv = nn.Conv2d(in_channels=1, out_channels=hidden_dim, kernel_size=1, bias=False) # [b, 1, 65, 65] -> [b, 64, 65, 65]
        self.agent_blocks = nn.Sequential(*[ConvNextBlock(hidden_dim) for _ in range(block_layers)]) # [b, 64, 65, 65]

        self.env_pwConv = nn.Conv2d(in_channels=1, out_channels=hidden_dim, kernel_size=1, bias=False)
        self.env_blocks = nn.Sequential(*[ConvNextBlock(hidden_dim) for _ in range(block_layers)]) # [b, 64, 65, 65]
